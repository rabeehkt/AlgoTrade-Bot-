...
from trading_bot.execution import OrderExecutor, evaluate_exit, mark_to_market, setup_logger
...
from trading_bot.universe import NIFTY50_SYMBOLS
...
class IntradayTradingBot:
    def __init__(self, creds: Credentials, runtime: BotRuntimeConfig, cfg: TradingConfig | None = None):
        ...
        if runtime.symbols:
            ...
            if invalid:
                raise ValueError(f"Only NIFTY50 symbols are allowed. Invalid: {invalid}")
    ...
    def _scan_and_trade(self, now: datetime) -> None:
        ...
        for symbol in self.symbols:
            ...
            signal = self.strategy.evaluate(symbol, df, now)
            if signal is None:
                continue
            ...
            self.risk_state.register_trade(symbol)
    ...
    def _manage_positions(self, now: datetime) -> None:
        for symbol, position in list(self.open_positions.items()):
            ...
            last = df.iloc[-1]
            should_exit, reason = evaluate_exit(position, last, now, self.cfg.force_exit)
            if not should_exit:
                continue
